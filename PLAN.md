...
- **Vercel Cron** for roster refresh + stats refresh + leaderboard precompute. Staying on the **Hobby plan** (free) means cron is limited to **once per day**; Session 4 adds an admin "refresh now" button hitting the same route on demand for same-day freshness during game weeks.
...
- **Player list** is one cached fetch (`/api/players`, ~100KB, revalidated every 15 min; static `players.json` as fallback) → typeahead filters **client-side**, zero per-keystroke server calls.
...
- The pool is the `players` table, kept current by the **nightly roster refresh** (`refreshRoster`, see Session 12): active QB/RB/WR/TE on an NFL roster.
- The picker reads it from **`/api/players`** (DB-backed, 15-min cache) so signings/trades reach users without a redeploy; `public/players.json` is the build-time fallback snapshot, regenerated by `npm run import:players`.
...
- **Roster churn never destroys data.** Players are only ever upserted or flagged `active = false` — never deleted — so an already-saved lineup keeps resolving (and keeps scoring) even if one of its players is cut.
...
- **Session 10 (feature/UX) — generator polish + hall of fame.** ✅ 21 Generator now defaults to **easy**, the 5th pick **auto-reveals** (no separate Reveal button), and the running total was removed. Exactly-21 runs can be added to a **speed leaderboard** (`generator_scores` table, migration `0011`; ranked by `durationMs`, separate easy/hard boards at `/play/leaderboard`) — client-timed and honour-system, bounded server-side (`submitGeneratorScore`). Two respin tokens (team + year) remain, gated to easy mode. Admins can **load past seasons from `/admin`** (`ingestPastSeason` → `ingestHistoricalSeason` with a reduced team-week set to fit the 60s budget; idempotent, one season per click) so the multi-year data can be backfilled without local DB access. Home page hides the marketing cards (example lineup + how-it-works) for signed-in users. Feedback email already flows through `sendFeedbackDigest` on submit — it just needs `AUTH_RESEND_KEY` + `FEEDBACK_NOTIFY_EMAIL` set. Regression test: "MY3".
- **Session 11 (UX/infra) — generator spin rework + Gmail email + scoreboard freshness.** ✅ 21 Generator now spins a random **team _and_ year** per slot (no season selector); easy mode gives one **team respin** (year locked) and one **year respin** (team locked, same franchise in a different loaded year) — see `app/play/roll.ts` `buildRandomBoard`/`buildTeamSlot`/`buildYearSlot`. Picking a player **auto-spins** the next slot (or reveals on the fifth), and run time is shown to the millisecond from the first spin. Feedback/cron emails now prefer **Gmail** (nodemailer + an App Password: `GMAIL_USER`/`GMAIL_APP_PASSWORD`) with Resend as a fallback (`lib/email.ts#sendMail`; nodemailer in `serverExternalPackages`). The scoreboard shows **stats last refreshed** in UK time (`getStatsRefreshedAt` → `max(leaderboard.computedAt)`, formatted `Europe/London`).

- **Session 12 (bugfix/infra) — rosters go stale.** ✅ The player pool was a one-off offseason import: `players.json` + the `players` table were written once by `npm run import:players` and nothing ever refreshed them, so every signing, trade and cutdown since was invisible (Stefon Diggs, Keenan Allen, Najee Harris and ~24 others simply weren't pickable), while ~237 players cut at the 53-man deadline still were. The daily cron only ingested **stats**, never the roster. Fixed on three fronts, all **additive — no row is ever deleted**, so `picks`, `player_week_stats`, `play_team` and `player_season_team` are untouched:
  - `lib/jobs/refresh.ts#refreshRoster` upserts today's rostered QB/RB/WR/TE (`lib/sleeper.ts#fetchRoster`) and flags anyone no longer rostered `active = false` with a null team. It deliberately omits `play_team` from the upsert so the frozen 2025 generator label survives (Sessions 7/8). A `MIN_ROSTERED = 400` guard in `fetchRoster` means a truncated Sleeper payload aborts rather than emptying the pool.
